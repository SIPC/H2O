...
| `preferred_locale` | `TEXT`    | nullable（用户界面语言偏好：`zh-CN` / `en-US`；NULL 表示跟随站点全局语言） |
...
| `ui_language`             | `uiLanguage`           | string  | `"zh-CN"`       |      | ✓    |

- `PATCH /api/admin/settings` 只接受白名单 key，并按每项默认值类型校验；`stats_retention_days` 范围是 1~365；`acme_ca_provider` 只能为 `letsencrypt` / `zerossl` / `custom`，自定义 CA 必须提供合法 Directory URL；`ui_language` 只能为 `zh-CN` / `en-US`。
...
### 完整 API 清单（55 路由文件，72 个 handler）

当前统计：GET 34 / POST 21 / PATCH 9 / DELETE 6 / PUT 2。
...
| PATCH | `/api/user/self/language`    | `requireUser` | 设置当前用户语言偏好（`inherit` / `zh-CN` / `en-US`） |
