...
function buildScript(params: InstallParams) {
  ...
  const lines: string[] = [
    ...
    "    return 0",
    "  fi",
    '  echo "缺少支持的服务管理器：需要 systemd 或 OpenRC" >&2',
    "  exit 1",
    "}",
    "",
    "ensure_command() {",
    ...
    "  else",
    '    echo "缺少创建系统用户命令: useradd/adduser" >&2',
    "    exit 1",
    "  fi",
    "}",
    "",
    ...
    "uninstall_h2o_node() {",
    "  require_root",
    '  echo "[h2o] 将停止并移除 h2o-agent 与 hysteria-server 服务、H2O 生成的配置文件和相关二进制。"',
    ...
    "  fi",
    "",
    '  echo "[h2o] 移除 h2o-agent"',
    "  rm -f /etc/systemd/system/h2o-agent.service /etc/init.d/h2o-agent",
  ...
  lines.push(
    ...
    "  fi",
    '  curl -A "H2O-Agent" -fsSL https://get.hy2.sh/ | FORCE_NO_SYSTEMD=2 HYSTERIA_USER=hysteria bash',
    "  ensure_system_user hysteria",
    "  create_openrc_hysteria_service",
    "fi",
    "",
    ...
    "else",
    '  echo "查看 agent 日志: tail -f /var/log/h2o-agent.log"',
    "fi"
  )
