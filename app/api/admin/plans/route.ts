...
export async function POST(request: Request) {
  ...
  try {
    ...
    const planRes = db
      .prepare(`INSERT INTO plans(name, traffic_limit_bytes, duration_days) VALUES (?, ?, ?)`)
      .run(body.name, body.trafficLimitBytes, body.durationDays)

    const planId = Number(planRes.lastInsertRowid)
    const insertPlanNode = db.prepare(`INSERT INTO plan_nodes(plan_id, node_id) VALUES (?, ?)`)
    ...
    return NextResponse.json({ ok: true, data: { id: planId, name: body.name } })
  } catch {
    db.exec("ROLLBACK")
    return NextResponse.json(
