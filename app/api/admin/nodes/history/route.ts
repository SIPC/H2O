...
export async function GET(request: Request) {
  ...
  const rows = db
    .prepare(
      `SELECT node_id, bucket_hour, tx_bytes, rx_bytes
       FROM node_hourly_traffic
       WHERE bucket_date = ?
         AND node_id IN (${placeholders})
       ORDER BY node_id ASC, bucket_hour ASC`
    )
  ...
  for (const row of rows) {
    const hourly = hourlyById.get(row.node_id)
    if (!hourly) continue
    if (
  ...
  const items = ids.map((nodeId) => {
    const hourly = hourlyById.get(nodeId) ?? buildEmptyHourly()
